document.addEventListener("DOMContentLoaded", async function () {
  ...
  const mobileTokens = (s) => {
    ...
    if (s === "compact") return { w: "min(92vw,340px)", pad: 10, img: 46, rad: 16, fs: 13 };
    if (s === "large") return { w: "min(92vw,420px)", pad: 14, img: 60, rad: 18, fs: 15 };
  ...
  function normHideKey(s){
    return String(s||"").toLowerCase().replace(/[\s_\-]+/g,"").replace(/[^a-z]/g,"");
  }
  function flagsFromNamesJson(raw){
    const arr = Array.isArray(raw) ? raw : (()=>{ try{ const v = JSON.parse(raw ?? "[]"); return Array.isArray(v)? v : (v? [v] : []); }catch{ return raw ? [raw] : []; }})();
  ...
  function parseLocationParts(entry){
    ...
    if (typeof entry === "object"){
      return {
        city:    safe(entry.city,"").trim(),
        state:   safe(entry.state||entry.province,"").trim(),
        country: safe(entry.country,"").trim()
      };
    }
    try{
  ...
  const shuffled = (arr) => { const a = arr.slice(); for (let i=a.length-1;i>0;i--){ const j=randInt(i+1); [a[i],a[j]]=[a[j],a[i]]; } return a; };
  function pickSmart(list, i, fb = "") {
    ...
    let tries = 0, v = "";
    while (tries < 7 && (!v || String(v).trim() === "")) { v = list[Math.floor(Math.random() * list.length)]; tries++; }
    return safe(v, fb);
  }
  ...
  function tryAnySvg(s) {
    ...
    try { const dec = atob(String(s)); if (/^<svg/i.test(dec)) return svgToDataUrl(dec); } catch {}
    return "";
  }
  ...
  // ======= date + time helpers =======
  const toDate = (v) => { try { return new Date(v); } catch { return null; } };
  const withinDays = (iso, days) => { const d = toDate(iso); if (!d) return false; const now = Date.now(); const ms = days*24*60*60*1000; return (now - d.getTime()) <= ms && d.getTime() <= now; };
  const relTime = (iso) => { const d = toDate(iso); if (!d) return ""; const diff = Math.max(0, Date.now() - d.getTime()); const m = Math.floor(diff/60000); if (m<1) return "just now"; if (m<60) return `${m} min ago`; const h = Math.floor(m/60); if (h<24) return `${h}h ago`; const dd = Math.floor(h/24); if (dd<7) return `${dd}d ago`; const wk = Math.floor(dd/7); if (wk<4) return `${wk}w ago`; const mo = Math.floor(dd/30); return mo<12 ? `${mo}mo ago` : `${Math.floor(dd/365)}y ago`; };
  const pad2 = (n) => String(n).padStart(2, "0");
  const formatAbs = (isoOrDate) => { const d = isoOrDate instanceof Date ? isoOrDate : toDate(isoOrDate); if (!d) return ""; return `${pad2(d.getDate())}/${pad2(d.getMonth()+1)}/${d.getFullYear()}, ${pad2(d.getHours())}:${pad2(d.getMinutes())}:${pad2(d.getSeconds())}`; };
  ...
  function getAnimPair(cfg, mode){
    ...
    if (a === "slide") {
      if (mode === "desktop") {
        // Desktop: slide based on left/right
        if (deskSide(cfg) === "right") return { inAnim: "fSlideInFromRight", outAnim:"fSlideOutToRight" };
        return { inAnim: "fSlideInFromLeft", outAnim:"fSlideOutToLeft" };
      }
      // Mobile: slide based on top/bottom
      const mp = normMB(cfg.mobilePosition || cfg.positionMobile || "bottom");
      if (mp === "top") return { inAnim:"fSlideInFromTop", outAnim:"fSlideOutToTop" };
      return { inAnim:"fSlideInFromBottom", outAnim:"fSlideOutToBottom" };
    }
  ...
  function renderFlash(cfg, mode, onDone) {
    ...
    const iSize = mode === "mobile" ? mt.img : 58, iRad = mode === "mobile" ? Math.round(mt.img * .17) : 12;
    ...
    const progCol = cfg.progressColor || cfg.titleColor || cfg.fontColor || "#22c55e";
    ...
    function autoClose() {
      ...
      setTimeout(() => { wrap.remove(); onDone && onDone("auto"); }, DUR.out + 20);
    }
  ...
  function renderRecent(cfg, mode, onDone) {
    ...
    const iSize = mode === "mobile" ? mt.img : 50, iRad = Math.round(iSize * .18);
    ...
    const locParts = [];
    if (!cfg.hideCity   && safe(cfg.city,"").trim())    locParts.push(safe(cfg.city,"").trim());
    if (!cfg.hideState  && safe(cfg.state,"").trim())   locParts.push(safe(cfg.state,"").trim());
    if (!cfg.hideCountry&& safe(cfg.country,"").trim()) locParts.push(safe(cfg.country,"").trim());
    let locFinal = locParts.join(", ");
    const anyLocHide = (cfg.hideCity || cfg.hideState || cfg.hideCountry);
    if (!locFinal && !anyLocHide && safe(cfg.location,"").trim()){
      locFinal = safe(cfg.location,"").trim();
    }
    if (nameText || locFinal){
      const nameHtml = nameText ? `<span style="font-weight:${fw};color:${ACCENT};">${nameText}</span>` : "";
      const locHtml  = locFinal ? `<span style="font-weight:${fw};color:${ACCENT};">${locFinal}</span>` : "";
    ...
    const boughtTxt = cfg.hideProductTitle
      ? "placed an order"
      : `${msgTxt} ${safe(cfg.productTitle,"") ? `&ldquo;${safe(cfg.productTitle,"")}&rdquo;` : "this product"}`;
    ...
    if (!cfg.hideTime) {
      ...
      line3.style.cssText = `font-size:${Math.max(10, (Number(cfg.baseFontSize) || 14) - 2)}px;opacity:.7;`;
    ...
    const barWrap = document.createElement("div"); barWrap.style.cssText = `height:4px;width:100%;background:transparent`;
    const bar = document.createElement("div");  bar.style.cssText = `height:100%;width:100%;background:${cfg.progressColor || ACCENT};animation:fomoProgress ${visibleMs}ms linear forwards;transform-origin:left;`;
  ...
  function createStream(name, renderer) {
    return {
      ...
      stop() { if (this.t) clearTimeout(this.t); this.t = null; try{ this.el && this.el.remove(); }catch{} },
      resize() {
  ...
  function createCombinedStream() {
    return {
      ...
      start(immediate=false){
        ...
        const first = this.seq[this.idx % this.seq.length];
        const firstDelay = immediate ? 0 : Math.max(0, +first.cfg.durationSeconds || 0);
        const showNext = (delay) => {
          this.t = setTimeout(() => {
            if (!this.seq.length) return;
            const item = this.seq[this.idx % this.seq.length]; // {type, cfg}
            const renderer = (item.type === "recent") ? renderRecent : renderFlash;
            this.el = renderer(item.cfg, "mobile", () => {
              const gap = Math.max(0, +item.cfg.alternateSeconds || 4);
  ...
  const IN_EDITOR = (ROOT?.dataset.designMode === 'true') || (window.Shopify && Shopify.designMode);

  if (IN_EDITOR) {
    const sampleFlash = [{
      title: "Flash Sale",
      location: "Winter Sale 15% Off",
      timeText: "Just now",
      uploadedImage: "",
      productUrl: "#",
    ...
    const sampleRecent = [{
      ...
      uploadedImage: "",
      timeText: "2 min ago",
      timeAbsolute: "27/10/2025, 13:16:34",
      mobilePosition: "bottom",
      durationSeconds: 0,
      ...
      hideName:false, hideCity:false, hideState:false, hideCountry:false,
      hideProductTitle:false, hideProductImage:false, hideTime:false
    }];

    Flash.seqDesktop = Flash.seqMobile = sampleFlash.slice();
    Recent.seqDesktop = Recent.seqMobile = sampleRecent.slice();

    // PREVIEW: decide overlap by positions on mobile
    if (isMobile() && hasMobileCollision(Recent.seqMobile, Flash.seqMobile)) {
      Combined.seq = interleaveList(
        Recent.seqMobile.map(cfg => ({type:"recent", cfg})),
        Flash.seqMobile.map(cfg => ({type:"flash",  cfg}))
      );
      Combined.start(true);
    } else {
      Flash.start(true);
    ...
    function place(el, side, baseBottom=24) {
      if (!el) return;
      const s = el.style;
      s.position = 'fixed'; s.zIndex = '2147483000'; s.margin = '0';
      s.bottom = baseBottom + 'px'; s.left = ''; s.right = '';
    ...
    function applyPositions() {
      requestAnimationFrame(() => {
        if (Combined.el) { place(Combined.el, 'right', 24); return; }
        place(Flash?.el, 'left', 24);
        place(Recent?.el, 'right', 24);
      });
    }
    window.FOMOIFY = window.FOMOIFY || {};
    window.FOMOIFY.preview = function () {
      try { Flash.el && Flash.el.remove(); } catch {}
      try { Recent.el && Recent.el.remove(); } catch {}
      try { Combined.el && Combined.el.remove(); } catch {}
    ...
    document.addEventListener('shopify:section:load', () => window.FOMOIFY.preview && window.FOMOIFY.preview());
    document.addEventListener('shopify:block:select', () => window.FOMOIFY.preview && window.FOMOIFY.preview());
    document.addEventListener('shopify:inspector:activate', () => window.FOMOIFY.preview && window.FOMOIFY.preview());
    applyPositions();
    window.addEventListener("resize", () => window.FOMOIFY.preview && window.FOMOIFY.preview());
    window.addEventListener("orientationchange", () => setTimeout(() => window.FOMOIFY.preview && window.FOMOIFY.preview(), 0));
    return;
  }

  /* ========== session check & fetch ========== */
  try {
    let sessionReady = false, retries = 0, maxRetries = 3, retryDelay = 2000;
    while (!sessionReady && retries < maxRetries) {
      try {
        ...
        if (sessionResponse.ok) {
          ...
          if (!sessionReady) { await new Promise(r => setTimeout(r, retryDelay)); retries++; }
        } else { retries++; if (retries < maxRetries) await new Promise(r => setTimeout(r, retryDelay)); }
      } catch { retries++; if (retries < maxRetries) await new Promise(r => setTimeout(r, retryDelay)); }
    }
    ...
    try {
      ...
      if (r.ok) { try { data = await r.json(); } catch { data = { records: [] }; } }
    ...
    for (const it of recs) {
      ...
      const match = (showType === "all" || showType === "allpage" || showType === pt);
      if (!match) continue;
      ...
      /* ---------- FLASH ---------- */
      if (it.key === "flash") {
        const n = Math.max(titlesArr.length||0, locsArr.length||0, timesArr.length||0, mbPosArr.length||0, 1);
        for (let i = 0; i < n; i++) {
      ...
      /* ---------- STRICT ORDERS MODE ---------- */
      const wantsOrders = (it.key === "orders") || (it.useOrders === true) || (String(it.useOrders) === "1");
      if (wantsOrders) {
        const daysWindowRaw = (it.orderDays ?? it.daysWindow ?? it.recentDays ?? 7);
        const daysWindow = Math.max(0, Number(daysWindowRaw) || 0);
        const limit = Math.max(1, Number(it.orderLimit || 30) || 30);

        if (daysWindow === 0) { continue; }

        try {
          const url = `${ORDERS_ENDPOINT_BASE}?shop=${encodeURIComponent(SHOP)}&days=${daysWindow}&limit=${limit}`;
          const ro = await fetch(url, { headers: { "Content-Type": "application/json" } });
          if (!ro.ok) continue;
          ...
          for (const o of orders) {
            ...
            const locJoin = (city || country || state) ? [city,state,country].filter(Boolean).join(", ") : pickSmart(locsArr, 0, "");
        ...
        } catch (e) { console.warn("[FOMO][orders] fetch failed", e); }
        continue;
      }
      ...
      if (isPd && includeCurrent && ch) {
        try {
          ...
          if (response.ok) {
            ...
            const locParts0 = parseLocationParts(pickSmart(locsArrRaw, 0, ""));
            recentConfigs.push({
              productTitle: p?.title || ch,
              name: pickSmart(titlesArr, 0, "Someone"),
              city:locParts0.city, state:locParts0.state, country:locParts0.country,
              ...
              timeText: pickSmart(timesArr, 0, safe(it.relativeTimeText, "")),
              timeAbsolute: nowAbs,
              mobilePosition: normMB(pickSmart(mbPosArr, 0, defaultMB), defaultMB),
      ...
      const n = Math.max(handlesArr.length||0, titlesArr.length||0, locsArrRaw.length||0, timesArr.length||0, mbPosArr.length||0, 1);
      for (let i = 0; i < n; i++) {
        ...
        try {
          ...
          const iconSrc = resolveIconForIndex(it, i);
          const nowAbs = formatAbs(new Date());
          const locPartsI = parseLocationParts(pickSmart(locsArrRaw, i, ""));
          recentConfigs.push({
            productTitle: (p?.title || handle || "Product"),
            name: pickSmart(titlesArr, i, "Someone"),
            city:locPartsI.city, state:locPartsI.state, country:locPartsI.country,
            location: formatLocationEntry(locPartsI),
            message: "recently bought",
            image: (p?.images && p.images[0]) || "",
            productUrl: (p?.url || (handle ? `/products/${handle}` : (includeCurrent && ch ? `/products/${ch}` : "#"))),
            uploadedImage: iconSrc,
            timeText: pickSmart(timesArr, i, safe(it.relativeTimeText, "")),
            timeAbsolute: nowAbs,
    ...
    for (const c of shuffled(recentConfigs)) {
      const key = [c.productTitle, c.name, (c.city||"")+(c.state||"")+(c.country||""), c.timeAbsolute || c.timeText].join("||");
    ...
    // - If mobile AND any same-position between streams -> alternate (Recent→Flash→…)
    // - Else run independently
    if (isMobile() && hasMobileCollision(RecentStream.seqMobile, FlashStream.seqMobile)) {
      Combined.seq = interleaveList(
      ...
      if (FlashStream.seqDesktop.length || FlashStream.seqMobile.length) {
        ...
        window.addEventListener("orientationchange", () => setTimeout(() => FlashStream.resize(), 0));
      }
      if (RecentStream.seqDesktop.length || RecentStream.seqMobile.length) {
        ...
        window.addEventListener("orientationchange", () => setTimeout(() => RecentStream.resize(), 0));
      }
    }

    // Re-evaluate on viewport and when collision status changes
    let lastIsMobile = isMobile();
    let lastCollision = isMobile() && hasMobileCollision(RecentStream.seqMobile, FlashStream.seqMobile);
    window.addEventListener("resize", () => reevaluateMode());
    window.addEventListener("orientationchange", () => setTimeout(() => reevaluateMode(), 0));

    function reevaluateMode(){
      const nowMobile = isMobile();
      const nowCollision = nowMobile && hasMobileCollision(RecentStream.seqMobile, FlashStream.seqMobile);
      if (nowMobile !== lastIsMobile || nowCollision !== lastCollision) {
        lastIsMobile = nowMobile; lastCollision = nowCollision;

        // stop all
        try{ Combined.stop(); }catch{}
        try{ FlashStream.stop(); }catch{}
        try{ RecentStream.stop(); }catch{}
  ...
  function interleaveList(a,b){
    const out=[]; let i=0;
    while(i<a.length || i<b.length){ if(i<a.length) out.push(a[i]); if(i<b.length) out.push(b[i]); i++; }
    return out;
  }
  function positionsSet(arr){
    const s = new Set();
    for (const c of (arr||[])) s.add(normMB(c.mobilePosition || c.positionMobile || "bottom"));
    return s;
  }
  function hasMobileCollision(recentList, flashList){
    const a = positionsSet(recentList);
    const b = positionsSet(flashList);
    for (const v of a) if (b.has(v)) return true; // any shared mobile position (top or bottom)
    return false;
  }
});
